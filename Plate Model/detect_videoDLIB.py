...
while(video.isOpened()):
    ...
    if tracker is not None: #We are currently tracking an object
        print("")
        #update the tracker and grap the position of the trackerd object
        tracker.update(rgbImage)
        pos = tracker.get_position()
        
        # unpate the position object
        startX = int(pos.left())
        startY = int(pos.top())
        endX = int(pos.right())
        endY = int(pos.bottom())
        ...
        if frameNumber >= 5:
            ...
            for plate in plates:
                ...
                xmax = plate[3][2]
                ymax = plate[3][3]
                if centerX > xmin and centerX < xmax and centerY > ymin and centerY < ymax:
                    goodTarget = True
                    break

            if goodTarget is False:
                tracker = None
    ...
    else:#We are not currentlly tracking an object
        if(bestPlate is not None):
            ...
            rect = dlib.rectangle(bestPlate[3][0],bestPlate[3][1],bestPlate[3][2],bestPlate[3][3])
            tracker.start_track(rgbImage, rect)
            # start OpenCV object tracker using the supplied bounding box
            # coordinates, then start the FPS throughput estimator as well
            fps = FPS().start()
            cv2.rectangle(frame,(bestPlate[3][0],bestPlate[3][1]), (bestPlate[3][2],bestPlate[3][3]),(255,0,255),1)
